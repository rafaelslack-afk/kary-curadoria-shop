  ...
  return (
    ...
    >
      <Image
        ...
        className={`${objectFit === "contain" ? "object-contain" : "object-cover"} will-change-transform`}
...
export function ProductClient({ product, variants, colorHexMap }: Props) {
  ...
  const [freteOpcoes, setFreteOpcoes] = useState<
    { id: number; name: string; preco: number; prazo: number }[]
  >([]);
  ...
  // ── Badges (reutilizado em mobile e desktop) ──────────────────────────────
  const badges = (
    <div className="absolute top-3 left-3 flex flex-col gap-1.5 z-10 pointer-events-none">
      {hasDiscount && (
        <span className="bg-kc-dark text-kc-cream text-[9px] tracking-[0.1em] px-2.5 py-1">
          {discountPct}% OFF
        </span>
      )}
      {allOutOfStock && (
        <span className="bg-white/90 text-kc-dark text-[9px] tracking-[0.14em] px-2.5 py-1">
          SEM ESTOQUE
        </span>
      )}
    </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 lg:px-6 py-6 md:py-8">
      ...
      <nav className="flex items-center gap-1.5 text-[10px] tracking-[0.14em] text-kc-muted mb-6">
        <Link
          href="/produtos"
          className="hover:text-kc-dark transition-colors flex items-center gap-1"
        >
          <ChevronLeft size={11} />
          Coleções
        </Link>
        {product.categories && (
          <>
            ...
            <Link
              ...
              className="hover:text-kc-dark transition-colors"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 md:items-start gap-8 md:gap-6 lg:gap-12">

        {/* ── Gallery ── */}
        <div>

          {/* Mobile gallery — mantido intacto */}
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {images.map((src, i) => (
                  <button
                    ...
                    className={`relative shrink-0 w-16 aspect-[3/4] bg-kc-cream overflow-hidden border transition-colors ${
                      selectedImage === i
                        ? "border-kc"
          ...
          {/* Desktop gallery — thumbnails verticais à esquerda */}
          <div
          ...
          >
            ...
            {images.length > 1 && (
              <div
                className="flex flex-col gap-2 overflow-y-auto shrink-0 [&::-webkit-scrollbar]:hidden"
              ...
              >
                {images.map((src, i) => (
                  <button
                    key={i}
                    onClick={() => setSelectedImage(i)}
                    className={`relative shrink-0 w-14 h-[70px] lg:w-16 lg:h-20 bg-kc-cream overflow-hidden border transition-colors ${
                      selectedImage === i
                        ? "border-kc"
                        : "border-kc-line hover:border-kc-muted"
                    }`}
            ...
            {/* Imagem principal */}
            <div className="relative flex-1 bg-kc-cream overflow-hidden">
              {images.length > 0 ? (
                <ZoomableImage
        ...
        {/* ── Info ── */}
        <div
          className="flex flex-col gap-5 md:sticky md:top-20 md:overflow-y-auto md:max-h-[calc(100vh-80px)]"
        >
          {product.categories && (
            <p className="text-[10px] tracking-[0.24em] text-kc-muted uppercase">
              {product.categories.name}
            </p>
          )}
          <h1 className="font-serif text-2xl lg:text-3xl font-medium text-kc-dark leading-snug">
            {product.name}
          </h1>

          <div className="flex items-baseline gap-3">
            {hasDiscount && (
              <span className="text-sm text-kc-muted line-through">
          ...
          {hasColors && (
            <div>
              <p className="text-[10px] tracking-[0.2em] text-kc-muted uppercase mb-3">
                Cor
                {selectedColor && (
                  <span className="ml-2 text-kc-dark font-medium">
                    — {selectedColor}
                  </span>
                )}
              </p>
                  ...
                  return (
                    ...
                    >
                      <span
                        ...
                        style={{
                          backgroundColor: colorHexMap[name] ?? "#cccccc",
                        }}
                      />
                      {!hasStock && (
          ...
          <div className="border border-kc-line bg-kc-cream/40 p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] tracking-[0.2em] text-kc-muted uppercase">
                Calcular Frete
              </p>
            </div>
            ...
            {freteOpcoes.length > 0 && (
              <ul className="space-y-2 pt-1">
                {freteOpcoes.map((op) => (
                  <li
                    key={op.id}
                    className="flex items-center justify-between bg-white border border-kc-line px-3 py-2.5"
                  >
                    ...
                    <div className="text-right">
                      ...
                      <span className="block text-[10px] text-kc-muted">
                        {op.prazo === 0
                          ? "Retirada"
                          : `${op.prazo} dia${op.prazo > 1 ? "s" : ""} úteis`}
                      </span>
                    </div>
                  </li>
                ))}
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <p style={{ fontSize: 9, color: "#B89070", margin: 0 }}>
                    Pagamento protegido
                  </p>
                </div>
              </div>
          ...
          {product.description && (
            <>
              <div className="h-px bg-kc-line" />
              <div>
                <p className="text-[10px] tracking-[0.2em] text-kc-muted uppercase mb-2">
                  Descrição
                </p>
                <p className="text-sm text-kc-dark/80 leading-relaxed whitespace-pre-line">
                  {product.description}
                </p>
              </div>
            </>
          ...
          {/* SKU */}
          {selectedVariant?.sku && (
            <p className="text-[9px] text-kc-muted tracking-wider">
              SKU: {selectedVariant.sku}
            </p>
          )}
        </div>
